...
```bash
cd backend
uv run agent_delta <group> <command> [options]
```

### Server Commands

```bash
uv run agent_delta server run              # Start dev server
uv run agent_delta server run --reload     # With hot reload
uv run agent_delta server run --port 9000  # Custom port
uv run agent_delta server routes           # Show all registered routes
```

### Database Commands

```bash
uv run agent_delta db init                  # Run all migrations
uv run agent_delta db migrate -m "message"  # Create new migration
uv run agent_delta db upgrade               # Apply pending migrations
uv run agent_delta db upgrade --revision e3f  # Upgrade to specific revision
uv run agent_delta db downgrade             # Rollback last migration
uv run agent_delta db downgrade --revision base  # Rollback to start
uv run agent_delta db current               # Show current revision
uv run agent_delta db history               # Show migration history
```
...
uv run agent_delta user create

# Create user non-interactively
uv run agent_delta user create --email user@example.com --password secret

# Create user with specific role
uv run agent_delta user create --email admin@example.com --password secret --role admin

# Create user with superuser flag
uv run agent_delta user create --email admin@example.com --password secret --superuser

# Create admin (shortcut)
uv run agent_delta user create-admin --email admin@example.com --password secret

# Change user role
uv run agent_delta user set-role user@example.com --role admin

# List all users
uv run agent_delta user list
```

### Taskiq Commands

```bash
uv run agent_delta taskiq worker                # Start worker
uv run agent_delta taskiq worker --workers 4    # 4 worker processes
uv run agent_delta taskiq worker --reload       # With auto-reload (dev)
uv run agent_delta taskiq scheduler             # Start periodic scheduler
```
...
```bash
uv run agent_delta cmd <command-name> [options]
```
...
uv run agent_delta cmd rag-ingest ./docs/guide.pdf

# Ingest a directory
uv run agent_delta cmd rag-ingest ./docs/

# Ingest recursively into a specific collection
uv run agent_delta cmd rag-ingest ./docs/ --collection knowledge --recursive

# Ingest with sync mode
uv run agent_delta cmd rag-ingest ./docs/ --sync-mode new_only
uv run agent_delta cmd rag-ingest ./docs/ --sync-mode update_only

# Skip replacing existing documents
uv run agent_delta cmd rag-ingest ./docs/ --no-replace
```
...
uv run agent_delta cmd rag-search "what is fastapi"

# Search a specific collection
uv run agent_delta cmd rag-search "deployment guide" --collection docs

# Get more results
uv run agent_delta cmd rag-search "deployment" --top-k 10
```
...
uv run agent_delta cmd rag-collections

# Show overall RAG system statistics
uv run agent_delta cmd rag-stats

# Drop a collection (with confirmation)
uv run agent_delta cmd rag-drop my_collection

# Drop without confirmation
uv run agent_delta cmd rag-drop my_collection --yes
```
...
uv run agent_delta cmd rag-source-remove <source-id>
uv run agent_delta cmd rag-source-remove <source-id> --yes  # Skip confirmation

# Trigger sync for a specific source
uv run agent_delta cmd rag-source-sync <source-id>

# Trigger sync for all active sources
uv run agent_delta cmd rag-source-sync --all
```
...
```bash
uv run agent_delta cmd my-command --name test
```
